...
Vec2i RotationCtrl::apply(Vec2i pos, Vec2i dims)  {
  switch (_rotation) {
    ...
    case 90:
      ...
      pos[0] = dims[1] - 1 - pos[0];
      std::swap(dims[0], dims[1]);
      break;
    case 180:
      // 180-degree rotation: reverse x and y
      pos[0] = dims[0] - 1 - pos[0];
      pos[1] = dims[1] - 1 - pos[1];
      break;
    case 270:
      std::swap(pos[0], pos[1]);
      pos[1] = dims[0] - 1 - pos[1];
      std::swap(dims[0], dims[1]);
      break;
    default:
      throw std::logic_error("Invalid rotation value! (Should be 0, 90, 180, or 270)");
...
Vec2i RotationCtrl::inverse_transformation(Vec2i pos, Vec2i dims)  {
  // Reverse the flips
  Vec2i rotated_dims = dims;
  if (_rotation == 90 || _rotation == 270) {
    std::swap(rotated_dims[0], rotated_dims[1]);
  }
  if (_fliplr && _flipud) {
    pos[0] = rotated_dims[0] - 1 - pos[0];
    pos[1] = rotated_dims[1] - 1 - pos[1];
  } else if (_fliplr) {
    pos[1] = rotated_dims[1] - 1 - pos[1];
  } else if (_flipud) {
    pos[0] = rotated_dims[0] - 1 - pos[0];
